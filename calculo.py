...
def aplicar_estilo_customizado():
    st.markdown("""
        ...
        .stTabs [data-baseweb="tab-list"] { gap: 15px; background-color: #ffffff; padding: 10px; border-radius: 10px; border: 1px solid #e9ecef; }
        .stTabs [data-baseweb="tab"] { background-color: #f8f9fa; border-radius: 5px; padding: 12px 25px; color: #495057; font-weight: 600; }
...
@st.cache_resource
def get_engine():
    # REMOVIDO: fast_executemany=False (Incompatível com pymssql)
...
# ==========================================
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/3233/3233513.png", width=100)
...
with tabs[0]:
    ...
    with col_r:
        ...
        if u:
            ...
            m1.metric("Custo", f"R$ {float(u['custo']):.2f}")
            m2.metric("Venda", f"R$ {float(u['venda']):.2f}")
            m3.metric("Lucro", f"R$ {float(u['lucro']):.2f}")
            
            msg = (f"Olá! Orçamento para *{u['produto']}* pronto. 🚀\n\n"
                   f"💰 *Valor:* R$ {float(u['venda']):.2f}\n"
                   f"📅 *Entrega:* {u['entrega']}\n"
                   f"📍 *Status:* {u['status']}\n\n"
                   f"Confirmamos?")
            if u['telefone']:
...
with tabs[1]:
    ...
    if not df.empty:
        ...
        st.divider()
        st.write("**🗄️ Histórico de Pedidos**")
        df['deletar'] = False
